...
const CafeDetail = ({ cafeData, onBack }) => {
    ...
    const loadMenuFromAPI = async () => {
        try {
            ...
            if (response && response.ok && response.data) {
                ...
                const storeMenu = response.data.filter(item => item.store_id === cafeData.id);
    ...
    if (!cafeData) {
        return (
            <div className="min-vh-100 d-flex align-items-center justify-content-center" style={{ backgroundColor: '#F6E0C4' }}>
                ...
                <div className="card mb-4 border-0 shadow">
                    <div className="row g-0">
                        <div className="col-md-8">
                            <img
                                src={cafeData.images?.[0]?.url || cafeData.image_url || "https://images.unsplash.com/photo-1501339847302-ac426a4a7cbb?w=800&h=400&fit=crop"}
                            ...
                            <div className="card-body h-100 d-flex flex-column justify-content-between">
                                <div>
                                    <div className="d-flex align-items-center mb-3">
                                        <img
                                            src={cafeData.logo_url || cafeData.image_url || "https://images.unsplash.com/photo-1554118811-1e0d58224f24?w=150&h=150&fit=crop"}
                                            alt="Logo"
                                            className="rounded-circle me-3"
                                            style={{ width: '60px', height: '60px', objectFit: 'cover' }}
                                            onError={(e) => {
                                                e.target.src = "https://images.unsplash.com/photo-1554118811-1e0d58224f24?w=150&h=150&fit=crop";
                                            }}
                                        />
                                        <div>
                                            ...
                                            <div className="d-flex align-items-center">
                                                ...
                                                <span className="ms-2 fw-bold">{(cafeData.rating || cafeData.total_points || 0).toFixed(1)}</span>
                                                <span className="ms-1 text-muted">({cafeData.review_count || 0} reseñas)</span>
                                            </div>
                                        </div>
                                    </div>

                                    <p className="text-muted mb-3">{cafeData.description || "Cafetería acogedora con excelente ambiente"}</p>

                                    <div className="mb-3">
                                        <span className={`badge px-3 py-2 ${cafeData.is_open ? 'bg-success' : 'bg-danger'}`}>
                    ...
                    <div className="row">
                        <div className="col-md-6 mb-4">
                            <div className="card border-0 shadow-sm">
                                ...
                                <div className="card-body">
                                    <p className="mb-2"><strong>Dirección:</strong> {cafeData.address || "Dirección no disponible"}</p>
                                    <div className="mt-3">
                                        <div
                        ...
                        <div className="col-md-6 mb-4">
                            ...
                            <div className="card border-0 shadow-sm mt-4">
                                ...
                                <div className="card-body">
                                    <p className="mb-2"><strong>Teléfono:</strong> {cafeData.phone || "Teléfono no disponible"}</p>
                                    <p className="mb-2"><strong>Email:</strong> {cafeData.email || "Email no disponible"}</p>
                                    <p className="mb-0"><strong>Web:</strong> {cafeData.website || "Sitio web no disponible"}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        {loading ? (
                            ...
                            cafeData.menu.map((menuSection) => (
                                <div key={menuSection.id} className="col-12 mb-4">
                                    <h4 className="mb-3" style={{ color: '#8B4513' }}>{menuSection.name}</h4>
                                    <div className="row">
                                                        ...
                                                        {category.products?.map(item => (
                                                            <div key={item.id} className="d-flex justify-content-between align-items-start mb-3 pb-3 border-bottom">
                                                                ...
                                                                <span className="fw-bold ms-3" style={{ color: '#8B4513' }}>
                                                                    {item.price ? formatCurrency(item.price) : 'Consultar'}
                                                                </span>
                                                            </div>
                                                        ))}
                                                    </div>
                            ...
                            <div className="col-12 text-center py-5">
                                ...
                                <p className="text-muted">Esta cafetería aún no ha subido su menú</p>
                                {!store.token && (
