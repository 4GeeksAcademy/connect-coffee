...
const Cloudinary = ({ preset = null, image_type = "store", owner_type = null, owner_id = null, label = "Imagen" }) => {
    ...
    const handleImageUpload2 = async (image_url) => {
        await setImageByType(store.token, image_url, image_type, owner_type, owner_id)
    }
    const uploadImage = async (e) => {            //2 Preparamos para recibir el evento al ejecutarse la función async
        ...
        try {
            ...
            dispatch({ type: "image_menu", payload: file.secure_url });
            await handleImageUpload2(file.secure_url);
